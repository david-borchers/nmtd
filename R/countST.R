...
#' 
#' @export
generate.countST=function(param, R, Tmax)
{
  lam=param[1]
  h=param[2]
  yt=matrix(0,R,2);
...
#' @title Evaluates the negative log-likelihood for model CountT:S with lambda constant
#'
#' @description
#' Evaluates the negative log-likelihood for model CountT:S, assuming constant lambda, 
...
#' 
#' @export
nll.countST=function(param, R, Tmax, dat)
{
  lam=exp(param[1])
  h=exp(param[2])
  loglik=matrix(0,R,1)
...
#' @title Generates time to detection for model CountT:S with lambda depending 
#' on a covariate.
#'
#' @description
#' Generates Poisson random variables for each of the \code{R} sites, corresponding to the 
...
#' 
#' # data for Binary:M
#' cnts=as.matrix(generate.countSTcov(paramt,R=Rsites,Tmax=Tsearch,covar=x))
#' str(cnts)
#' head(cnts)
#' 
#' @export
generate.countSTcov=function(param,R,Tmax,covar)
{
  b0=param[1]
  b1=param[2]
  h=param[3]
...
#' @title Evaluates the negative log-likelihood for model CountT:S with lambda depending 
#' on a covariate.
#'
#' @description
#' Evaluates the negative log-likelihood for model CountT:S, assuming that lambda 
...
#' animals in the site depends linearly (assumed to be the same for all occasions).
#' 
#' @return Returns the negative log-likelihood function for model CountT:S evaluated at 
...
#' 
#' # data for Count:ST
#' cntm=as.matrix(generate.countSTcov(paramt,R=Rsites,Tmax=Tsearch,covar=x))
#' 
#' init.paramt=c(b0t, b1t, log(ht))
#' nll = nll.countSTcov(param=init.paramt, R=Rsites, Tmax=Tsearch,dat=cntm, covar=x)
#' nll
#' 
#' # optimize
#' fit.countSTcov=optim(init.paramt,nll.countSTcov,R=Rsites,Tmax=Tsearch,dat=cntm,covar=x)
...
#' 
#' @export
nll.countSTcov=function(param,R,Tmax,dat,covar)
{
  b0=param[1]
  b1=param[2]
  h=exp(param[3])
