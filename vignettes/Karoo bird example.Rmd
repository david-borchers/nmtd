...
```{r}
bdata$surveynumber <- as.numeric(substr(bdata$survey_id, 11,12))

bdat.binT1 <- matrix(NA, nrow = length(unique(bdata$pentad)), ncol = max(bdata$surveynumber)) # this doesn't work because surveys aren't numbered consecutively...
